<template>
  <div class="mt-10">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <div v-for="books in paginatedBooks" :key="books.id" class="bg-white p-4 rounded-lg shadow-md">
        <div class="relative">
          <img :src="books.image" alt="Books Image" class="w-full h-64 object-cover rounded-lg mb-4" />
        </div>
        <div class="flex items-center">
          <h3 class="text-lg font-semibold text-gray-800 mb-2 truncate max-w-xs">{{ books.title }}</h3>
        </div>
        <div>
